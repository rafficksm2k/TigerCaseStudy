...
function Container(props) {
      ...
      if (typeof FileReader != "undefined") {
        ...
        reader.onload = function (e) {
          ...
          storeContext.dispatch({
            ...
            filteredProduct: csvArray,
          });
          setTableViewProducts(csvArray.slice(firstPageIndex, lastPageIndex));

          let headerArr = [];
          Object.keys(csvArray[0]).map((key) => headerArr.push(key));
  ...
  function onUpdate(updatedValue, event) {
    if (event === "update") {
      const currentIndex = store.products.findIndex(
        (product) =>
          product.StoreID === updatedValue.StoreID &&
          product.SKU === updatedValue.SKU,
      );
  ...
  const onSearch = (key, value) => {
    ...
    const filteredData = store.products.filter((product) =>
      product[key].toLowerCase().includes(value.toLowerCase()),
    );
    storeContext.dispatch({
      type: SET_TABLE_SIZE,
      tableSize: filteredData.length,
    });
  ...
  function defaultTabelViewData(value) {
    ...
    if (value) {
      ...
      storeContext.dispatch({
        type: SET_TABLE_SIZE,
        tableSize: store.products.length,
      });
    }
  }
  ...
  function onPageChange(page) {
    ...
    setTableViewProducts(
      store.filteredProduct.slice(firstPageIndex, lastPageIndex),
    );
  }
    ...
    <>
      ...
      {!store.showTable ? (
        ...
        <>
          ...
          <hr />
          <Table
            productHeader={store.header}
            products={tableViewProducts}
      ...
      {store.editTable && (
        <Edit selectedProduct={selectedProduct} onUpdate={onUpdate} />
      )}
    </>
  );
}
